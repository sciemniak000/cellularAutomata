...
public class WireWorldGriddy extends WireWorld {
    public WireWorldGriddy(HashMap<Integer, ? extends CellState> cellz, int size_x, int size_y, CellNeighborhood moor_or_neuman){
        ...
        if(cells.get(coords.get(0)) == WireElectronState.VOID){
            ...
            for(int i = 1; i < coords.size(); i++){
                ...
                if(!(x >= width || y >= height)){
                    ...
                    if(cells.get(y*1000+x) == WireElectronState.ELECTRON_HEAD){
                        counter++;
                    }
                }
            }
