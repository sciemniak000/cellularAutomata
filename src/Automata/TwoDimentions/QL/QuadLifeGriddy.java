...
public class QuadLifeGriddy extends QuadLife {
    public QuadLifeGriddy(HashMap<Integer, ? extends CellState> cellz, int size_x, int size_y, CellNeighborhood moor_or_neuman){
    ...
    protected CellState getChanges(ArrayList<Integer> coords) {
        ...
        for(int i = 1; i < coords.size(); i++) {
            ...
            if (!(x >= width || y >= height)) {
                ...
                if (cells.get(1000*y+x) != QuadState.DEAD) {
                    switch ((QuadState) cells.get(1000*y+x)) {
                        case RED:
                            red++;
                            break;
                        case BLUE:
